...
- **Stage preview** sits above "Drift to Sleep" on the map: the faces waiting on the selected page, their health, the wave count, and whether you have read it. A night is a real commitment for the player this game is for; they should see what they are agreeing to.
- **Losing is a story beat.** `Models/DefeatNote.swift` holds the text for each ending — a title, one line of story naming the page, and one concrete thing to try — and `DefeatAdviceView` renders that last part on all three losing cards. Every losing card also offers "Close the book" alongside "Try Again": losing must never be a room with one door. Keep the advice actionable and in the storybook voice; it is the only coaching this game gives.
