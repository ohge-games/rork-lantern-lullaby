...
- **Letting a card go**: dropping any card on the lantern calls `releaseSelectedCard`, which discards it and lowers Lucidity by `GameRules.releaseRelief` without resolving effects or accruing strain. This is the guaranteed out when a hand holds no Relax card.
